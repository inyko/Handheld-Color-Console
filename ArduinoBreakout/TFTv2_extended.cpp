...
void TFT::drawChar( INT8U ascii, INT16U poX, INT16U poY,INT16U size, INT16U fgcolor)
{
  ...
  for (int i =0; i<FONT_X; i++ ) {
    ...
    for(INT8U f=0;f<8;f++)
    {
      if((temp>>f)&0x01)
      {
        fillRectangle(poX+i*size, poY+f*size, size-1, size-1, fgcolor);
      }
    }
  }
}
...
void TFT::drawChar( INT8U ascii, INT16U poX, INT16U poY,INT16U size, INT16U fgcolor, INT16U bgcolor )
{
  ...
  for (int i =0; i<FONT_X; i++ ) {
    ...
    for(INT8U f=0;f<8;f++)
    {
      ...
      {
        fillRectangle(poX+i*size, poY+f*size, size-1, size-1, bgcolor);
      }
...
void TFT::scrollArea(uint16_t top, uint16_t bottom) {
  
  sendCMD(0x33);
  
  sendDATA(top >> 8);
  sendDATA(top);
  sendDATA((320 - top - bottom) >> 8);
  sendDATA(320 - top - bottom);
  sendDATA(bottom >> 8);
  sendDATA(bottom);
}
